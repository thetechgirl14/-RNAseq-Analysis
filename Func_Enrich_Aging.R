...
# Importing data
res <- read.csv(paste0(path, "differential_expression_aging.csv"), header = TRUE)
...
geneList_id <- merge(sigGenes, row_genes, by = "gene_id")
geneList_id

# Extract gene Ids from DEseq2 results
geneList <- geneList_id[["gene_entrez_id"]]
geneList
enrichResult <- enrichKEGG(gene = geneList, organism = "hsa", pvalueCutoff = 0.05)
enrichResult
