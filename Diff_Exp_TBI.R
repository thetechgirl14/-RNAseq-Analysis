...
dds <- DESeqDataSetFromMatrix(countData = round(counts_filt), colData = coldata, design =~ num_tbi_w_loc)
...
ggplot(data = as.data.frame(res), aes(x = log2FoldChange)) + 
  geom_histogram(bins = 100)
...
write.csv(sig_genes_with_id, paste0(path,"differential_expression_tbi.csv"))